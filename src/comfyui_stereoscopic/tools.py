...
class BuildVariantIndex:
    ...
    RETURN_NAMES = ("VariantSelectionPath", "yaml_path", "used_seed",)
    CATEGORY = "Stereoscopic"
    FUNCTION = "build_variant_index"
    ...
    def build_variant_index(self, yaml_path: str, VariantIndexValues: str = "", random_seed: int = 0, seed_offset: int = 0):
        ...
        return (VariantSelectionPath, yaml_path, used_seed)
...
class GetVariant:
    """
    Traverses a hierarchical YAML profile structured like:
    ...
    The node flattens all `props` along the VariantSelectionPath and
    computes the overall probability of the selected variant path.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "yaml_path": ("STRING", {"multiline": False, "default": ""}),
                "VariantSelectionPath": ("LIST",),
            }
        }

    RETURN_TYPES = ("DICT", "FLOAT",)
    RETURN_NAMES = ("properties", "probability",)
    FUNCTION = "load_yaml_properties"
    CATEGORY = "Stereoscopic"

    # ------------------------------------------------------------

    def load_yaml_properties(self, yaml_path: str, VariantSelectionPath: list):
        """Loads YAML and extracts properties along the VariantSelectionPath."""
        if not yaml_path or not os.path.exists(yaml_path):
            raise FileNotFoundError(f"YAML file not found: {yaml_path}")
