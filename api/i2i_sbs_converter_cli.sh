...
then
	...
	TARGETPREFIX="$TARGETPREFIX""_fullsbs"
	...
	then
		...
		then
			...
			# "$DEPTH_MODEL_CKPT" $DEPTH_RESOLUTION $depth_scale $depth_offset $blur_radius
			echo -ne $"\e[91m" ; "$PYTHON_BIN_PATH"python.exe $SCRIPTPATH -i "$INPUT" -o "$INTERMEDIATE" --input-type i2i --model "depth-anything/Depth-Anything-V2-Small-hf" --depth-scale $depth_scale --depth-offset $depth_offset --blur-radius $blur_radius --symmetric ; echo -ne $"\e[0m"
