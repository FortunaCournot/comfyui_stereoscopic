  ...
  rm -rf -- output/vr/tasks/intermediate
    ...
    if [[ "$timeout" =~ ^[0-9]+$ ]]; then
      if [[ "$secs" -gt "$timeout" ]]; then
        ...
        cmd='Get-Process -Name python | Where-Object { ($_.PagedMemorySize64/1KB) -gt 20000000 } | ForEach-Object { Write-Output ("Killing {0} (pid {1}) PM(K)={2}" -f $_.ProcessName,$_.Id,[math]::Round($_.PagedMemorySize64/1KB)); Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue }'
