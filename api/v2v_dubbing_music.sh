...
#
# dubbes a base video (input) by mmaudio and places result under ComfyUI/output/vr/dubbing/music folder.
#
...
then
	...
	mkdir -p output/vr/dubbing/music/intermediate
	
	`"$FFMPEGPATHPREFIX"ffprobe -hide_banner -v error -select_streams v:0 -show_entries stream=codec_type,codec_name,bit_rate,width,height,r_frame_rate,duration,nb_frames -of json -i "$INPUT" >output/vr/dubbing/music/intermediate/probe.txt`
	
	temp=`grep duration output/vr/dubbing/music/intermediate/probe.txt`
	temp=${temp#*:}
	temp="${temp%\"*}"
    temp="${temp#*\"}"
    temp="${temp%,*}"
	...
	PROGRESS=" "
	if [ -e input/vr/dubbing/music/BATCHPROGRESS.TXT ]
	then
		PROGRESS=`cat input/vr/dubbing/music/BATCHPROGRESS.TXT`" "
	fi
	regex="[^/]*$"
	...
	TARGETPREFIX_CALL=vr/dubbing/music/intermediate/${TARGETPREFIX%.*}
	TARGETPREFIX=output/${TARGETPREFIX_CALL}
	FINALTARGETFOLDER=`realpath "output/vr/dubbing/music"`
	...
	POSITIVEPATH="$CONFIGPATH/dubbing_music_positive.txt"
	NEGATIVEPATH="$CONFIGPATH/dubbing_music_negative.txt"
	...
	do
		...
		if [[ $COUNT -eq 0 ]] ; then
			...
			#mkdir -p ./input/vr/dubbing/music/done
			#mv -fv $INPUT ./input/vr/dubbing/music/done
			exit 1
		else
			for f in *.flac; do
	...
	if [ -e "$DUBBINGDIR/merged.flac" ]; then
		...
		while [ -e output/vr/dubbing/music/"$TARGETFILEBASE""_dub"$(printf %03d "$((10#$n))" )".mp4" ]; do n=$((n+1)); done
		mv -f -- $DUBBINGDIR/dubbed.mp4 output/vr/dubbing/music/intermediate/"$TARGETFILEBASE""_dub"$(printf %03d "$((10#$n))" )".mp4"
		mv -vf -- output/vr/dubbing/music/intermediate/"$TARGETFILEBASE""_dub"$(printf %03d "$((10#$n))" )".mp4" "$FINALTARGETFOLDER"		
		...
		mv -fv $INPUT ./input/vr/dubbing/music/error
		exit 0
	fi
