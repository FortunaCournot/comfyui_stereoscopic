...
then
	...
	for f in "$SEGDIR"/segment*.mp4 ; do
		...
		then
			...
			echo -ne $"\e[91m" ; "$PYTHON_BIN_PATH"python.exe $SCRIPTPATH "$DEPTH_MODEL_CKPT" $DEPTH_RESOLUTION $depth_scale $depth_offset $blur_radius "$f" "$SBSDIR_CALL"/sbssegment  ; echo -ne $"\e[0m"
		else
			[ $loglevel -ge 0 ] && echo -ne "+ $f2...       \r"
		fi
	done
	...
	then
		...
		if [[ ! $TESTAUDIO =~ "[STREAM]" ]]; then
			echo "    nice "$FFMPEGPATHPREFIX"ffmpeg -hide_banner -loglevel error -y -f concat -safe 0 -i list.txt -c copy -an output2.mp4" >>"$SBSDIR/concat.sh"
			echo "    nice "$FFMPEGPATHPREFIX"ffmpeg -hide_banner -loglevel error -y -i output2.mp4 $SETMETADATA -vcodec libx264 -x264opts \"frame-packing=3\" -force_key_frames \"expr:gte(t,n_forced*1)\" $TARGETPREFIX"".mp4" >>"$SBSDIR/concat.sh"
		else
		...
		echo "    [ -e \"$EXIFTOOLBINARY\" ] && \"$EXIFTOOLBINARY\" -m -tagsfromfile \"$INPUT\" -ItemList:Title -ItemList:Comment -creditLine -xmp:rating -SharedUserRating -overwrite_original $TARGETPREFIX"".mp4 && echo \"ItemList and Rating tags copied.\"" >>"$SBSDIR/concat.sh"
		...
		echo "    mkdir -p $CWD/input/vr/fullsbs/error" >>"$SBSDIR/concat.sh"
