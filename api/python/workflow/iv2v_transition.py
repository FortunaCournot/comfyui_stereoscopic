...
if len(sys.argv) != 7 + 1 and len(sys.argv) != 8 + 1:
   print("Invalid arguments were given ("+ str(len(sys.argv)-1) +"). Usage: python " + sys.argv[0] + " apifile StartImagePath ControlVideoPath OutputPathPrefix Length Prompt blend_factor [ColorImagePath]")
else:
    try:
    ...
    try:
        ...
        prompt["217"]["inputs"]["blend_factor"] = sys.argv[7]
        if len(sys.argv) == 9:
            prompt["220"]["inputs"]["image"] = sys.argv[8]
        else:
            prompt["178"]["inputs"]["image"] = sys.argv[2]
    except Exception as e:
