...
if [ ! -e "$CONFIGPATH"/"rebuild_autoforward.sh" ] ; then
	...
	echo "echo '[duration<10:tvai=true:width<7681]interpolate' >output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "echo '[duration<10:tvai=true]tasks/vlimit-1080p' >>output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "#echo '[duration<10:vram>52]interpolate' >>output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	...
	echo "echo '[duration<10]interpolate'        >output/vr/tasks/vlimit-1080p/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "echo '[duration<10]interpolate'        >output/vr/tasks/vlimit-720p/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
