...
if [ ! -e "$CONFIGPATH"/"rebuild_autoforward.sh" ] ; then
	...
	echo "echo '[tvai=true:width<7681]interpolate' >output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "echo '[tvai=true]tasks/vlimit-1080p' >>output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "#echo '[vram>52][duration<10]interpolate' >>output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "echo '[vram>13]tasks/vlimit-1080p' >>output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
	echo "echo '[vram>6]tasks/vlimit-720p' >>output/vr/fullsbs/forward.txt"  >>"$CONFIGPATH"/"rebuild_autoforward.sh"
